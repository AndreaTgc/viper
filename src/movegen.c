...
void genPseudoLegalMoves(Position *pos, MoveList *moves) {
    ...
    // Handle pawn moves
    int starting_square, ending_square;
    ...
    while (pawns) {
        ...
        if (SQUARE_AS_BIT(starting_square) & starting_rank) {
            ...
            if (!SQUARE_IS_VALID(ending_square))
                break;
            Bitboard dpush = SQUARE_AS_BIT(ending_square) &
        ...
        if (SQUARE_IS_VALID(ending_square)) {
            Bitboard spush = SQUARE_AS_BIT(ending_square) &
                             ~(pos->occupancies[0] & pos->occupancies[1]);
    ...
    while (knights) {
        ...
        Bitboard attacks = knight_attacks[starting_square] &
